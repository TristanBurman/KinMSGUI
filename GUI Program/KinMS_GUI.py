...
def AOI(cube):
    ...
    -------
    Start: int
        Initial channel index for the area of intrest 
        
    Stop: int
        Final channel index for the area of intrest 
    '''
...
def ExtractVarFromStr(String):
    ...
    ----------
    String: str
        Equation input in string format
    ...
    Bounds: list
        List of bounds corresponding to Input values 
    '''
    Index = []
    i = String.find('|')
    while i >= 0 :
...
def EditUserString(String, ValueArray):
    ...
    ValueArray: np.ndarray
        Array of values to replace variables in String
        
    Returns
    -------
    NewString: str
        New string with ValueArray values in place of string variables 
    '''
    Index = []
    i = String.find('|')
    while i >= 0 :
...
def SwitchMode(self, Win):
    """
    Switch Mode from 'Read Mode' to 'Custom Model' or vice versa
    
    Parameters
    ----------
    Win: str
        String indicating intended mode to switch to     
    """
    if Win == 'Read Mode':
...
def VPExamples(self, Type):
    """
    Fills in VP text box with example equations
    
    Parameters
    ----------
    Type: str 
        String indicating intended equation to input
    """
    if Type == 'Arctan':
...
def SBExamples(self, Type):
    """
    Fills in SB text box with example equations
...
class ReadWin:
    ...
    def Fitting(self):
        ...
        def Chisquare(Values):
            ...
            ----------
            Values: np.ndarray
                Array of guesses to calculate Chi Square
            """
            #Seperate out Guess
...
class CustomWin:
    ...
    def Run(self):
        ...
        try:
            ...
            self.StatusBox.delete(0, TK.END)
            self.StatusBox.insert(0, 'Disk thickness needs to be defined for more plots')  
